...
@jit(nopython=True, parallel=False)
def gradientLoop(varGradient, fluxIntegralTotal, varField,
                 areaInvD, NS, sid, fluxIntegral):
    NC = len(areaInvD)
    for elem in range(NC):
        optimizedComputeGradientFV3_Private(varGradient[:,
                                                        elem],
                                            fluxIntegralTotal,
                                            varField,
                                            areaInvD[elem],
...
@jit(nopython=True, parallel=False)
def optimizedComputeGradientFV3_Private(
        varGradient, fluxIntegralTotal, varField, areaInvD, sid, fluxIntegral):
    ...
    for pp in range(NS):
        ...
        fluxIntegralTotal = np.add(
            fluxIntegralTotal, varAvg * fluxIntegral[:, pp])
...
@jit(nopython=True, parallel=False)
def buildConvexHullOfCellsAroundCell(
        jj, NP, pdex, pdexp, varStenDex, convHullSten, thisStencil):
...
@jit(nopython=True, parallel=False)
def computeDualEdgesMap(NS, convHullSten, pdex, dualEdgeMap, radius,
                        cellCoords, boundaryAngles, boundaryNorm, sid, fluxIntegral):
    for pp in range(NS):
        ...
        # Compute the weighted average of the two cell values AT the shared
        # edge location
...
def precomputeGradientFV3Data_Private(
        NC, NP, jj, varCon, varCoords, varStenDex, radius, cellCoords):
    SF = np.float64
    ...
    pdex_size, convHullSten_size = buildConvexHullOfCellsAroundCell(
        jj, NP, pdex, pdexp, varStenDex, convHullSten, thisStencil)
...
class ComputeGradientFV:
    ...
    def precomputeGradientFV3Data(self):
        ...
        if not self.cachedData:
            ...
            for elem in range(NC):
                self.sid[elem, 0:self.NS[elem], :] = np.array(
                    results[elem][1], dtype='i4')
    ...
    def computeGradientFV3(self, varField):
        ...
        runInParallel = False
        # If we have not actually precomputed necessary datastructures already,
        # let us do it now
        if runInParallel:
            if not self.cachedData:
        ...
        if runInParallel:
            ...
            computepool.close()
            computepool.join()
            varGradient = np.reshape(
                np.array(results, dtype=SF).T, (nc, varField.shape[0]))
        else:
