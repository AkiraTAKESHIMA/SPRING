...
def loadMeshJacobians(mesh_file, varJacoName, SpectralElement):
    if SpectralElement:
        ...
        try:
            ...
            print(
                'ERROR: PRE-PROCESSING FOR JACOBIANS NOT AVAILABLE ON TARGET MESH FILE!')
            sys.exit()
...
def loadFieldGradient(gradCtx, varField, varConn, varCoord,
                      varConStenDex, jacobians, numCells, SpectralElement):
    ...
    if SpectralElement:
        ...
        gradField = computeGradientSE(
            varField, varConn, varCoord, 4, jacobians)
    else:
        numDOFS = numCells
...
if __name__ == '__main__':
    ...
    # Max number of remap iteration solutions to use in order to compute the
    # metrics
    maxRemapIterations = 1000
    ...
    jacobiansS = loadMeshJacobians(
        mesh_fileS,
        varJacoName,
        isSourceSpectralElementMesh)
    jacobiansT = loadMeshJacobians(
        mesh_fileT,
        varJacoName,
        isTargetSpectralElementMesh)
    ...
    if includeGradientMetrics:
        ...
        start = time.time()
        gradSCtx = ComputeGradientFV(
            'Source', varConS, varCoordS, varConStenDexS, nprocs)
        gradSCtx.precomputeGradientFV3Data()
        print(
            'Time taken to precompute gradient datastructures for source grid: ',
            time.time() - start)
        start = time.time()
        gradTCtx = ComputeGradientFV(
            'Target', varConT, varCoordT, varConStenDexT, nprocs)
        gradTCtx.precomputeGradientFV3Data()
        print(
            'Time taken to precompute gradient datastructures for target grid: ',
            time.time() - start)
    ...
    for fieldName in fieldNames:
        ...
        # Print out a table with metric results. Let us print progress during
        # iteration progress
        printProgressBar(
            0,
        ...
        def metrics_per_field_dimension(iteration):
            ...
            varS2T, varT2S = loadDataField(
                ncFieldFileHnd, fieldName, iteration)

            # %% Computing all metrics...

            # Global conservation metric
            massS2T, massST, L_g = computeGlobalConservation(
                varConS, varConT, varSS, varS2T, varST, areaS, areaT, jacobiansS, jacobiansT, 
                isSourceSpectralElementMesh, isTargetSpectralElementMesh)
            #massSS, massS2T, L_g = computeGlobalConservation(
            #    varConS, varConT, varSS, varS2T, varST, areaS, areaT, jacobiansS, jacobiansT, 
            #    isSourceSpectralElementMesh, isTargetSpectralElementMesh)
            ...
            L_1, L_2, L_inf = computeStandardNorms(
                varConT, varS2T, varST, areaT, jacobiansT, isTargetSpectralElementMesh)
...
#                    isTargetSpectralElementMesh)
            ...
            if includeGradientMetrics:
                ...
                H1, H1_2 = computeGradientPreserveMetrics(
                    varConT, gradientsOnTM, varsOnTM, areaT, jacobiansT, isTargetSpectralElementMesh)

                df.loc[iteration, 'H12T'] = H1_2
                df.loc[iteration, 'H1T'] = H1

                # Gradient preservation checks on source grid
                gradT2S = loadFieldGradient(
                    gradSCtx,
                    varT2S,
                    varConS,
                    varCoordS,
                ...
                H1, H1_2 = computeGradientPreserveMetrics(
                    varConS, gradientsOnSM, varsOnSM, areaS, jacobiansS, isSourceSpectralElementMesh)
        ...
        results = iterationpool.starmap(
            metrics_per_field_dimension, zip(
                range(
                    maxRemapIterations + 1)))
        iterationpool.close()
        iterationpool.join()
        ...
        print(
            '\nFinished computing metrics for',
            fieldName,
            'in',
            time.time() -
            runtimeS,
        ...
        print(
            'Writing metrics data for',
            fieldName,
            'field to',
            outputMetricsFile)
        print(
            '\n\t\t\t\tTABLE OF REMAPPING ITERATION METRICS FOR Field=',
            fieldName,
            '\n')
        print(dataMetrics)
