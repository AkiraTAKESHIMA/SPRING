...
if __name__ == '__main__':
    ...
    #
    dir_flwdir = os.path.join(lconst.dir_tmp[step]['self'], '1min_flwdir')
    ...
    org = os.path.join(os.path.abspath(lconst.dir_tmp[lutil.istep('rasterize_OGCM')]['self']), 'mask.bin')
    dst = os.path.join(dir_flwdir, 'lndmsk.bin')
    os.symlink(org, dst)

    dir_gcmmap = os.path.join(lconst.dir_tmp[step]['self'], 'gcmmap')
    os.makedirs(dir_gcmmap, exist_ok=True)

    dir_tmp = os.path.join(lconst.dir_tmp[step]['self'], 'tmp')
    os.makedirs(os.path.join(dir_tmp, '1min'), exist_ok=True)
    os.makedirs(os.path.join(dir_tmp, 'map/1min'), exist_ok=True)

    dir_map = os.path.join(lconst.dir_tmp[step]['self'], 'map')
    os.makedirs(dir_map, exist_ok=True)

    #
    # Make a setting file for FLOW
    #
    f_params = os.path.join(lconst.dir_tmp[step]['self'], 'params.txt')
    fp = open(f_params,'w')
    fp.write(f'\
{cnf["RM"]["ncx"]}  ! nXX\n\
{cnf["RM"]["ncy"]}  ! nYY\n\
...
0.d0  ! square of the earth\'s eccentricity\n')
    ...
    os.chdir(lconst.dir_tmp[step]['self'])
    ...
    for i, prog in enumerate([
      ...
      'calc_inpmat',
        ...
        f_log = f'{lconst.dir_log[step]["self"]}/{i+1:02d}_{prog}.out'
        f_err = f'{lconst.dir_log[step]["self"]}/{i+1:02d}_{prog}.err'
        util.exec_program(prog, f_conf, f_log, f_err)
