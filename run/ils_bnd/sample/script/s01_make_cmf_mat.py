...
def run():
    ...
    f_conf = os.path.join(settings['directory']['set']['01'],'define_mat.conf')
...
'.format(dir_out=settings['directory']['tmp']['01'])\
+'\
\n\
  idx_miss: -9999\n\
[end]\n\
...
'.format(dir_out=settings['directory']['tmp']['01']))
    ...
    fp.write(conf.block_options(settings['options']['earth']))
