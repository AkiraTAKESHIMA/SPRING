...
def conf_remap_latlon(tileName, landType, dataName, data_in, matsiro, directory, options):
    ...
    dir_out_this = os.path.join(directory['tmp']['02'], dataName)

    s = head(landType, tileName, dir_out_this)

    s += block_gs_latlon(data_in, twest, teast, tsouth, tnorth)

    s += block_gs_raster(landType, directory['tmp']['01'], matsiro, dxi, dxf, dyi, dyf)
...
def conf_remap_modis(tileName, landType, dataName, data_in, matsiro, directory, options):
