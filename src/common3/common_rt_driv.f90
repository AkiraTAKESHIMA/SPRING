...
subroutine driv_make_rt_raster_polygon(&
  ...
  use common_area_raster_polygon, only: &
        ini_common_area_raster_polygon => initialize, &
        fin_common_area_raster_polygon => finalize
  use common_rt_raster_polygon, only: &
        ini_common_rt_raster_polygon => initialize, &
        fin_common_rt_raster_polygon => finalize  , &
  ...
  ! Initialize
  call ini_common_area_raster_polygon(ar, bp, rt%vrf_source%dval_miss)
  call ini_common_rt_raster_polygon(ac, bc)
  ...
  do iaz = 1, ar%nZones
    ...
    do ibz = 1, bp%nZones
      ...
      ! Make a remapping table
      !---------------------------------------------------------
      call make_rt_raster_polygon(a, b, rt, opt_sys, opt_earth)
      !---------------------------------------------------------
      !
      !---------------------------------------------------------
      if( bp%nZones > 1 )then
  ...
  ! Finalize
  call fin_common_rt_raster_polygon()
  call fin_common_area_raster_polygon()
