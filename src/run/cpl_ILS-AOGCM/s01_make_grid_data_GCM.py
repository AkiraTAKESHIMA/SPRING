...
def make_grid_data(cnf, step, update_data, gsName):

    c = cnf[k_gs][gsName]
    dir_tmp = os.path.join(os.getcwd(), lconst.dir_tmp[step], gsName)
    ...
    dir_tmp = os.path.join(lconst.dir_tmp[step], gsName)
...
def run(update_data):
    ...
    make_grid_data(cnf, step, update_data, 'AGCM')
    make_grid_data(cnf, step, update_data, 'OGCM_ocean')
