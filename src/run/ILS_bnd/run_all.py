...
if args.step is None:
    for step in range(env.step_max+1):
        if step == istep('make_rt'):
            lconst.scripts[step].run(update_data, args.data, args.land, args.tile)
        else:
    ...
    for step in range(args.step):
        if step == istep('make_rt'):
            lconst.scripts[step].run(False, args.data, args.land, args.tile)
        else:
            lconst.scripts[step].run(False)
    if args.step == istep('make_rt'):
        lconst.scripts[args.step].run(update_data, args.data, args.land, args.tile)
    else:
