...
!module MOD_SFMT
module lib_random_sfmt  ![2024/12/13 Takeshima]
    implicit none

    ![2024/12/13 Takeshima]
    private
...
!    use MOD_SFMT
    ...
    if (btest(idx,0))then
        ...
        gen_rand32 = int(ibits(psfmt64(idx/2),32,32), 4) ![2024/12/13 Takeshima]
    else
        !gen_rand32 = ibits(psfmt64(idx/2),0,32)
        gen_rand32 = int(ibits(psfmt64(idx/2),0,32), 4) ![2024/12/13 Takeshima]
    end if
    idx = idx + 1
end function
!#endif
...
!    use MOD_SFMT
    ...
    do i=1, N32-1
        ...
        psfmt32(i) = int(ibits( temp64, 0, 32 ), 4) ![2024/12/13 Takeshima]
    end do
